...
# ■サービス概要
色をもっと身近に楽しもう。「Color Garalley」は「あなたの身の回りにある好きな色の組み合わせ」を登録するカラーパレットアプリです。非デザイナーや非フロントエンジニア向けに特化し、色の知識がなくても誰でも簡単に色を登録することができます。実際にその配色を用いたデザインのプロトタイプをリアルタイムで表示し、「デザインにそのまま使える色選び」をサポートします。
...
そこで本アプリでは「自分の周りにあるなんとなく好きなもの・好きなデザインを色の模倣で表現する」をメインコンセプトに設定します。
...
- **カラーパレットの投稿**
    ...
    ユーザーは「日常生活の中にある色の組み合わせ」を投稿できます。どのような色の組み合わせを投稿しても問題ないですが、非デザイナーが色を見つけるのに困らないように、アプリ自体のコンセプトはこの前提に基づいています。
...
- **色の選び方**
    ...
    1. **カラーサークルから手探りで似た色を見つけていく**
    視覚的に似た色を選んでいく方法です。確実に同じ色を選ぶのは難しいかもしれませんが、自発的に色に触れる経験が得られます。
...
- **面積比率の設定**
    ...
    選んだ3色は5%単位で面積比率を設定できます。例えば、某コーラロゴの場合、赤色が50%、白色が20%、青色が30%くらいで構成されているため、カラーパレットでもこの比率で登録可能です。
...
- **配色の活用**
    ...
    自身の開発やデザインにそのまま使用できる配色を登録・確認できます。
